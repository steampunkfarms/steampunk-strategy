...
- 🤖 **2026-03-20:** Real-Cost Impact Lines step 7 — seasonal awareness complete. TARDIS exposes `/api/costs/seasonal-baselines`. Studiolo syncs SeasonalCostBaseline table, `loadCostMapForMonth()` overrides current costs with seasonal typicalPrice (bermuda-hay → hay-bale mapping). Backfill route uses cost-at-gift-date for single-gift donors, current costs for multi-gift. Schema: SeasonalCostBaseline model + seasonalFlag on CostOfCareItem. 1 file created in TARDIS, 4 files modified in Studiolo.
- 🤖 **2026-03-20:** (STRIPE-WEBHOOK) Stripe `invoice.payment_failed` webhook listener for Studiolo — complete. Listens for `invoice.payment_failed` to create FrictionAlerts surfaced on dashboard "At Risk" card. Also listens for `invoice.paid` to auto-resolve alerts and populate "Recovered QTD".
- 🤖 **2026-03-20:** (STRIPE-EXPAND) Expand Stripe webhook to accept all event types — complete. Webhook now accepts all Stripe event types with structured logging for unhandled types. Processes `invoice.payment_failed`, `invoice.paid`, `customer.subscription.deleted`, `customer.subscription.updated`, `charge.refunded`.
- 🤖 **2026-03-18:** Facebook SSO + Studiolo Donor Linking — complete. Enabled Facebook OAuth on Rescue Barn and Cleanpunk Shop (same Supabase project, shared identity). OAuth callback now looks up user email in Studiolo donor API and stores `studiolo_donor_id` on profiles table (best-effort, 3s timeout). Apple Sign In scaffolded (disabled, awaiting Apple Developer approval). Cleanpunk callback now also upserts to shared profiles table so Cleanpunk-first users get a profile row. Migration 020 adds `studiolo_donor_id` column. 4 files in Rescue Barn (2 new, 2 modified), 2 files in Cleanpunk Shop (2 modified).
- 🤖 **2026-03-13:** (20260313-cross-site-type-drift) Cross-Site Type Drift Detection (Option B) — complete. Canonical types file with 22 interfaces covering 11 internal API endpoints. Drift-check script (`scripts/check-type-drift.ts`) with regex-based structural parser. Per-repo contract files in 5 repos (Studiolo 11, Postmaster 6, Rescue Barn 2, Cleanpunk 1, Strategy 5 interfaces). All contracts verified aligned. 8 created, 0 modified files across 5 repos.
- 🤖 **2026-03-13:** (20260313-tier1-lint-fix) Fix obsolete `next lint` → `eslint .` in steampunk-strategy package.json. Next.js 16 removed built-in lint command.
- 🤖 **2026-03-13:** (20260313-reference-docs-full-refresh) Reference Docs Full Refresh — complete. Created tardis-reference.md (41 models, 89 routes) and orchestrator-reference.md (6 models, 25 managed crons). Updated postmaster-reference.md (+12 models, +20 routes, newsletter/social/chronicle/vet sections), rescuebarn-reference.md (+crons, Cogworks, newsletter integration), studiolo-reference.md (TARDIS integration), cleanpunk-shop-reference.md (TARDIS + Orchestrator deps). Updated CLAUDE.md (Reference Library, route structure, cron jobs, cross-site integration). 2 created, 5 modified files in 1 repo (steampunk-strategy).
- 🤖 **2026-03-12:** (20260312-credential-registry-dashboard-cron) Credential Registry Dashboard + Cron Integration — complete. Dashboard at /credentials with 6 KPI gauges + expandable table. Sidebar nav entry (KeyRound, Command group). Bridge conditional alert widget. Health-check cron extended with checkCredentialHealth() for Anthropic + Stripe API key verification. PATCH/rotate API routes with audit logging. 4 created, 3 modified files in 1 repo (steampunk-strategy).
- 🤖 **2026-03-11:** Cancel Square Billing — complete. Square Services ($15/mo marketing) cancelled. Square Online cancelled via Website settings → Site preferences. Seeds fundraiser site archived to /Users/ericktronboll/Projects/seeds-fundraiser-archive.md before closure.
- 🤖 **2026-03-11:** (#100) Zeffy Phase-Out — complete. Zapier webhook kept for existing monthly donors. No new traffic routed to Zeffy; all new donors go to rescuebarn.steampunkfarms.org/donate (Stripe). No further action needed until last monthly donor re-platforms.
- 🤖 **2026-03-11:** (#25) PayPal Gmail Enrichment — verified complete. `lib/gmail-enrichment.ts` + `lib/gmail-parsers/paypal.ts` handle 3 PayPal email formats, extracting donor notes (AI-tagged), addresses, fees, subscription details. PayPal Intelligence card live on Commerce page. PayPal API sync runs daily at 7am UTC; Gmail enrichment is manual/backfill only.
- 🤖 **2026-03-11:** (#23) Gmail Label Discovery — verified complete. `GET /api/gmail/import/labels` lists all user labels with message counts. `getLabelId()` helper used by all channel scanners. "Discover Labels" tab in GmailImportPanel. Manual UI tool, not a scheduled job.

- 🤖 **2026-03-10:** (20260310-health-check-cron) Fleet Health-Check Cron — `/api/cron/health-check` route with 6 parallel check categories: Vercel fleet deployment status, production URL reachability (6 sites), TARDIS database connectivity, cross-site API auth verification (Studiolo/Postmaster/Cleanpunk), data freshness (transactions, RaiseRight imports), cron freshness audit (AuditLog scan for recent cron_run entries). Results logged to AuditLog. Auth: CRON_SECRET/INTERNAL_SECRET with timing-safe comparison. 1 new file in 1 repo (steampunk-strategy).
- 🤖 **2026-03-09:** (COG-GROUND) Cogworks AI Grounding — Stop Hallucination. Grounded all Cogworks AI generation with resident data from Postmaster API + Universal Voice Guardrails (Layer 1). All 3 Claude calls (body, excerpt, YouTube copy) include guardrails + resident context. YouTube transcript processor validates residentSlugs against known residents. Newsletter confidence gate deprioritizes Cogworks posts with unrecognized animal names (0.3x engagement penalty). Sanity deltas: replaced nonexistent `residents` Supabase table with Postmaster API fetch; replaced `species_groups` name matching with body-text regex extraction. 1 new + 6 modified files across 2 repos (Rescue Barn + Postmaster).
- **2026-03-09:** (RIG-1) Resident Multi-Image Gallery — ResidentImage Prisma model with tags/season/freshness tracking, CRUD API routes, Vercel Blob drag-drop upload, admin gallery UI on resident detail page, public API extension (?fields=images&imageLimit=N), Storm image selector with weighted scoring (season/tags/freshness/useCount), Dear Humans auto-select pilot, migration of 9 existing imageUrl values. 8 new + 3 modified files in 1 repo (Postmaster). Deferred: Rescue Barn gallery UI, Cleanpunk consumption, engagement-weighted scoring, AI image tagging.
- **2026-03-08:** (SYNC-1) Studiolo Donor → Newsletter Subscriber Sync — Cross-site donor-to-subscriber pipeline. Studiolo GET endpoint returns subscribable donors (excludes Lane B/Opus, null emails, placeholder emails). Fire-and-forget sync helper integrated into 11 donor creation flows (webhooks, imports, Patreon sync). Rescue Barn POST endpoint upserts into newsletter_subscribers with timing-safe auth. One-time backfill script. 2 new + 11 modified in Studiolo, 2 new in Rescue Barn, 1 working spec.
- **2026-03-08:** (PROTOCOL-2) Two-Actor Model — Retired Codex mandatory QA role. QA absorbed into CC Post-Execution QA Protocol (6-step checklist). Two-actor framework: CChat (strategist, Tier 3) + CC (executor + QA, Tier 0/1/2). Added External Review Exception Table, Optional Tools section. Deprecated docs/CODEX.md. Updated Protocol Change Procedure (no CODEX.md sync). Cascaded to all repos. Docs-only change in 1 repo (steampunk-strategy), 2 files modified (CLAUDE.md, docs/CODEX.md).
- **2026-03-08:** (20260308-faq-knowledge-base) FAQ-1 FAQ Knowledge Base + Admin UI + Seed Content + Public AI FAQ Agent — Database-driven FAQ system with 3 Supabase tables (faq_categories, faq_entries, faq_conversations) + RLS, 30 seed FAQ entries across 7 categories from Video FAQ + Facebook automation, admin CRUD with bulk publish/unpublish, category management, conversation analytics with escalation filter, public accordion FAQ page with search, Claude Haiku AI FAQ chatbot ("Ask the Barn") as floating widget on all public pages, conversation logging with helpful/not-helpful feedback, rate limiting (10 msg/5 min). Migration 014, seed script, 4 admin pages, 1 public page rewrite, 1 API route, 3 chat widget components. 15 files across 1 repo (Rescue Barn).
- **2026-03-07:** (20260307-yt1-youtube-cogworks-import) YT-1 YouTube Transcript -> Cogworks Importer — YouTube Data API v3 channel video listing (paginated, max 1000), youtubei.js transcript extraction with youtube-transcript fallback, Claude Sonnet transcript processing (clean prose, species classification, excerpt, social caption, donor hook, SEO description), TipTap body builder with embedded YouTube player. CLI bulk import script (--dry-run, --limit, --since), on-demand API route with INTERNAL_SECRET/admin auth, admin UI page. All imports as drafts. 6 new + 3 modified files in 1 repo (Rescue Barn).
- **2026-03-07:** (20260307-commons1-rescue-commons) COMMONS-1 Rescue Commons Foundation — Authenticated community platform with three zones (Bray blog discussions, Corral general forum, Punkyard fun/memes). Claude Haiku AI moderation (toxicity 0-1, auto-approve/review/reject), strike system (3=suspend, 5=ban), threaded comments (depth 3), Supabase Realtime, upvotes, flags, admin moderation queue, blog post discussion integration. Migration 013, 7 API endpoints, 6 components, 7 pages. 24 new + 2 modified files in 1 repo (Rescue Barn).
- **2026-03-07:** (20260307-hug1-atelier-digest) HUG-1 Atelier Lane HUG Digest — Monthly personalized donor letters using 5-layer voice engine. Per-donor content matching (animal bonds via DonorAnimal junction, species interests from boolean flags), mood selection (grateful/celebratory/tender/warm), 25-day recency guard, anti-CTA enforcement in AI prompt, Postmaster content feed API (Chronicle + Animal Chronicles + Cogworks + campaigns + milestones). Compose HUG button on donor detail page. Monthly cron (15th, 14:00 UTC). 8 files across 2 repos (Studiolo 7 + Postmaster 1).
- **2026-03-07:** (20260307-news3-seasonal-publication) NEWS-3 Seasonal Publication — Quarterly "State of the Barn" editorial letter (2000-3000 words) with census data, impact transparency, campaign arc storytelling, caretaker voices (scored journal excerpts), seasonal framing (4 frames: spring/summer/fall/winter), winter year-in-review. TARDIS impact metrics with local fallback, content aggregation from 5 sources, quarterly cron (Jan/Apr/Jul/Oct), manual compose trigger, Compose Seasonal button. 11 files across 1 repo (Postmaster).
- **2026-03-07:** (20260307-caretaker-chronicle) CHRON-1 Caretaker Chronicle System — JournalEntry model, Whisper voice transcription, Twilio SMS gateway, Claude auto-tagging (mood/animals/tags), ChronicleButton omnipresent on all admin pages with per-site auth proxies (Supabase for Rescue Barn, NextAuth for Studiolo + TARDIS). Chronicle feed page with stats, filters, timeline. 27 files across 4 repos (Postmaster + Rescue Barn + Studiolo + TARDIS).
- **2026-03-07:** (20260307-news2-composition-engine) NEWS-2 Weekly Dispatch + Monthly Barn Letter — AI composition engine with content selector (Cogworks + Chronicle + campaigns + milestones), animal continuity tracker (rotation across issues), 8 weighted format variations (weekly), narrative arc structure (monthly), engagement echo (community comments), subject line engine (3 options: curiosity/character/sensory), preview text generator, cron schedules (Thursday weekly, 1st monthly), manual compose trigger, Compose Now buttons on editorial review page. 12 files across 1 repo (Postmaster).
- **2026-03-07:** (20260307-newsletter-foundation) NEWS-1 Newsletter Foundation — Resend batch send pipeline, HTML email templates (weekly/monthly/seasonal), subscriber schema upgrade with cadence preferences and token-based unsubscribe, newsletter issue CRUD + admin dashboard, Cogworks query APIs for content ingestion. Postmaster: NewsletterDraft model, cross-site content ingestion library, editorial review page, dispatch to Rescue Barn. 23 files across 2 repos (Rescue Barn + Postmaster).
