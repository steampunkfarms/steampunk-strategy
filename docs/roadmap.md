...
**Progress (2026-03-10):** Invoice → CostTracker promotion pipeline deployed. TARDIS auto-extracts line items from parsed invoices (vet, feed, farrier) via AI and promotes per-unit costs to CostTracker. `GET /api/costs/all` exposes latest costs. Studiolo `sync-cost-of-care` cron created. Backfill script ready for 99 existing documents.
**Remaining:**
...
- [x] (DATA-FIX) Data gap fixes: bulk document-to-transaction conversion, MBOX parser, Gmail backfill, gap audit script, favicon — see `docs/handoffs/20260307-tardis-data-gap-fixes.md`
  - 🤖 **2026-03-07:** Extracted shared transaction-creation function, built bulk convert API + UI, MBOX email parser script, Gmail API backfill endpoint, document gap audit script, TARDIS favicon. 11 files created/modified. (task completed)
- [x] (VEN-1) Vendor Intelligence Page — drill-down with financial KPIs, price trends, program allocation, product-species mapping, donor-paid bills, AI insight — see `docs/handoffs/20260307-vendor-intelligence-page.md`
  - 🤖 **2026-03-07:** Rewrote vendor [id] page from edit form to 6-section intelligence dashboard. Created analytics API, Claude insight API. Vendor list cards now drill down to detail. Edit form moved to /edit subroute. 7 files created/modified. (task completed)
- [x] (EXP-1) Expenses Page Analytics Upgrade — KPI strip, vendor breakdown, COA + program allocation, monthly trend, feed cost tracker, pending review, budget vs actual, AI insight — see `docs/handoffs/20260307-tardis-expenses-bi-phase1.md`
  - 🤖 **2026-03-07:** Replaced flat summary bar with 8-section analytics dashboard. Created expense analytics API, Claude insight API. Vendor breakdown links to vendor detail pages. 5 files created/modified. (task completed)
- [x] (BI-3) Layer 3 — Strategic Intelligence Engine: AI insight generator, idea incubator, predictive forecasting, scenario modeling, board/grant report packs — see `docs/handoffs/20260307-bi-strategic-layer3.md`
  - 🤖 **2026-03-07:** Built Strategic Intelligence Engine with Claude-powered insight generation (5-7 categorized cards), linear regression forecasting with seasonal adjustment, interactive scenario builder with 5 templates and Claude narrative, idea incubator, and one-click board/grant HTML report export. 15 files created/modified. (task completed)
...
- [x] (STRIPE-WEBHOOK) Stripe `invoice.payment_failed` webhook listener for Studiolo
  → 🤖 **2026-03-20:** Completed — listens for `invoice.payment_failed`, creates FrictionAlert, surfaces on dashboard "At Risk" card. Also listens for `invoice.paid` to auto-resolve alerts and populate "Recovered QTD".

- [x] (STRIPE-EXPAND) Expand Stripe webhook to accept all event types
  → 🤖 **2026-03-20:** Completed — webhook now accepts all event types, logs unhandled types, processes `invoice.payment_failed`, `invoice.paid`, `customer.subscription.deleted`, `customer.subscription.updated`, `charge.refunded`.
