// Shared logic for creating a Transaction from a parsed Document
// Used by both single create-transaction route and bulk-create-transactions route
...
export interface CreateTransactionOverrides {
  vendorSlug?: string;
  categorySlug?: string;
  date?: string;
  amount?: number;
...
export async function createTransactionFromDocument(
  documentId: string,
  overrides?: CreateTransactionOverrides,
): Promise<CreateTransactionResult> {
  const doc = await prisma.document.findUnique({
    where: { id: documentId },
    include: { vendor: true, transactions: true },
  });
  ...
  // Dedup: skip if document already has a linked Transaction
  if (doc.transactions.length > 0) {
    throw new Error(`Document already linked to transaction ${doc.transactions[0].transactionId}`);
  }
  ...
  try {
    ...
    throw new Error('Invalid extractedData JSON');
  }
  ...
  // --- ProductSpeciesMap learning loop ---
